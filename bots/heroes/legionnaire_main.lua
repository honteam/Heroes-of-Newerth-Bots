...
local function legoItemBuilder()
		...
		if not tItemDecisions.Lane then		
				...
				if tLane then
						...
						if tLane.sLaneName == "lane_jungle" then
...
function jungleUtility(botBrain)
	if (HoN.GetRemainingPreMatchTime() and HoN.GetRemainingPreMatchTime() > 14000) or core.tMyLane.sLaneName ~= 'lane_jungle' then -- don't try if we have a lane!
		return 0
	end
	return 21
end
...
local function PositionSelfUtilityOverride(botBrain)
	if (core.tMyLane and core.tMyLane.sLaneName ~= 'lane_jungle') then
...
local function PreGameUtilityOverride(botBrain)
	if (not (core.tMyLane and core.tMyLane.sLaneName == 'lane_jungle')) then
