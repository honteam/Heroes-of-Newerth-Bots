...
function object:onthinkOverride(tGameVariables) --This is run, even while dead. Every frame.
	...
	if tControllableUnits ~= nil and core.NumberElements(tControllableUnits.AllUnits) > 0 then
		tControllableUnits = tControllableUnits.AllUnits
	else 
		tControllableUnits = nil
	end	
	
	if (nNumMinions == 0 and unitSelf:IsAlive() and tControllableUnits ~= nil) then
		object.tMinions = {}
		i = 1 --keep track of array index
		
		for key, unit in pairs(tControllableUnits) do		
			if (unit:GetTypeName() == "Pet_Bephelgor_Ability3" and unit:GetOwnerPlayerID() == unitSelf:GetOwnerPlayerID()) then
	...
	if (object.nCurSkip >= object.nMinionSkip and table.getn(object.tMinions) > 0) then--don't skip
		...
		---------------------------------
			...
			for i = 1, nNumMinions do
				local unitMinion = object.tMinions[i]
				if (unitMinion ~= nil and unitMinion:IsValid() and unitMinion:IsAlive()) then
					core.OrderAttackPosition(botBrain, unitMinion, destPos, false, false)--attackmove
				else
					tremove(object.tMinions, i) 
					i = i - 1 
					nNumMinions = table.getn(object.tMinions)
