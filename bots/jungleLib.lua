...
function jungleLib.assess(botBrain)
	...
	if time then
		...
		if (mins == 0 and secs == 30) or (mins ~= jungleLib.minutesPassed and mins ~= 0) then --SPAWNING
			for _, jungleSpot in pairs(jungleLib.jungleSpots) do
				if (not jungleSpot.corpseBlocking) then --it won't spawn with corpse in way.
					jungleSpot.stacks = 1 --assume something spawned. If not, it will be removed later if not.
				end
				jungleSpot.corpseBlocking = false
			end
	...
	for _, jungleSpot in pairs(jungleLib.jungleSpots) do
		...
		if (HoN.CanSeePosition(jungleSpot.pos))then
			jungleSpot.creepDifficulty = 0
			local nUnitsNearCamp = 0
			local uUnits = HoN.GetUnitsInRadius(jungleSpot.pos, 600, core.UNIT_MASK_ALIVE + core.UNIT_MASK_UNIT)
			...
			if jungleSpot.stacks ~= 0 and nUnitsNearCamp == 0 then --we can see the camp, nothing is there.
				if (debug) then BotEcho("Camp " .. jungleSpot.description .. " is empty. Are they all dead? "..jungleLib.jungleSpots[i].stacks) end
				if secs > 37 then
					jungleSpot.corpseBlocking = true
				end --perhaps add to this. This is a corpse check.
				jungleSpot.stacks = 0
			end
			if (nUnitsNearCamp ~= 0 and jungleSpot.stacks == 0 ) then --this shouldn't be true. New units should be made on the minute.
				if (debug) then BotEcho("Camp "..jungleSpot.description.." isn't empty, but I thought it was... Maybe I pulled it too far?") end
				jungleSpot.stacks = 1
			end
		end
	end
...
function jungleLib.getNearestCampPos(pos, minimumDifficulty, maximumDifficulty, side)
	...
	for i = 1, #jungleLib.jungleSpots do
		local jungleSpot = jungleLib.jungleSpots[i]
		if side == nil or jungleSpot.side == side then
			local dist = Vector3.Distance2DSq(pos, jungleSpot.pos)
			local difficulty = jungleSpot.difficulty + jungleSpot.creepDifficulty
			if dist < nClosestSq and jungleSpot.stacks ~= 0 and difficulty > minimumDifficulty and difficulty < maximumDifficulty then
				nClosestSq = dist
				nClosestCamp = i
			end
		end
	end
	if (nClosestCamp ~= -1 and jungleLib.jungleSpots[nClosestCamp].stacks > 0) then
		return jungleLib.jungleSpots[nClosestCamp].pos, nClosestCamp
	end
	return nil
end
